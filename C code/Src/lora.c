...
lora_write_reg(int reg, int val)
{
   ...
   HAL_SPI_TransmitReceive(&hspi1, out, in, 2, 20);
...
lora_init(void)
{
   ...
   uint8_t i = 0;
     while(i++ < TIMEOUT_RESET) {
      ...
      HAL_UART_Transmit(&huart2, &version, 1, 20);
...
lora_send_packet(uint8_t *buf, int size)
{
  ...
  // printf("Lora write mode set done...\n");
   while((lora_read_reg(REG_IRQ_FLAGS) & IRQ_TX_DONE_MASK) == 0)
   {
      HAL_Delay(2);
     // printf("Lora waiting for confirmation...\n");
   }
